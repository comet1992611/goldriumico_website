		...
		<div class="col-md-12">
			<div class="portlet light bordered">
                                ...
                                <div class="portlet-body form">
                                    <form role="form" method="POST" action="{{url('admin/gsettings/email')}}/{{$gsettings->id}}" enctype="multipart/form-data">
                                    	{{ csrf_field() }}
										{{method_field('put')}}
                                        <div class="form-body">
                                            <div class="form-group">
                                                <label>Email Sent Form</label>
                                                <input type="email" name="emailSender" class="form-control input-lg" value="{{$gsettings->emailSender}}">
                                            </div>
                                            <div class="form-group">
                                                <label>Email Message</label>
                                                <textarea class="form-control" name="emailMessage" rows="10">
                                                	{{$gsettings->emailMessage}}
                                                </textarea>
                                            </div>
                                        </div>
